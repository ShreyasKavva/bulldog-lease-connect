...
export async function fetchMyTours(userId: string) {
  const { data, error } = await supabase
    .from("tour_bookings" as any)
    .select("*, listing:listings(id,title,area,photos,user_id), poster:profiles_public!tour_bookings_poster_id_fkey(id,name,avatar_url,banner_color), subletter:profiles_public!tour_bookings_subletter_id_fkey(id,name,avatar_url,banner_color)")
