...
async function fetchUserListings(userId: string, activeOnly: boolean) {
  ...
  let q: any = supabase.from("listings").select(activeOnly ? PUBLIC_LISTING_COLUMNS : "*").eq("user_id", userId)
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-surface p-3 shadow-card-md">
      ...
      >
        ...
        {(() => {
          const v = listing.view_count ?? 0;
          const parts = [
            v > 0 ? `${v.toLocaleString()} ${v === 1 ? "view" : "views"}` : null,
